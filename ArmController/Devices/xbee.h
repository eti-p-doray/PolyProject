...
public:
    ...
    public:
        ...
        void getFrame(QScopedPointer<ResponseFrameData>& responseFrameData);
        template <typename T> void getFrame(QScopedPointer<T>& responseFrameData)
        {
            frameData_->getType();
            responseFrameData.reset( (T*)frameData_.take() );
            frameData_.reset(new InvalidFrameData);
        }

        template <typename T> T getFrame()
        {
            T ans = *(T*)frameData_.take();
            frameData_.reset(new InvalidFrameData);
            return ans;
        }
    ...
    private:
        QScopedPointer<ResponseFrameData> frameData_;
        QByteArray buffer_;
    };

    XBee() = default;
    XBee(QScopedPointer<QSerialPort>& port);
    ~XBee();

    void read(ResponsePacket* packet);
    void write(RequestPacket packet);

    QSerialPort* getPort() {return port_.data();}
